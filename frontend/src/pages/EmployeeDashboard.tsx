...
export default function EmployeeDashboard() {
  ...
  const canSubmit = invalidIssues === 0

  function handleSubmit() {
    setSubmitNotice(
      canSubmit
        ? 'График отправлен на проверку.'
        : 'Сначала исправьте предупреждения по неделям.',
    )
  }
          ...
          <div className="grid grid-cols-3 gap-2 sm:min-w-[440px]">
            ...
            <MetricTile
              ...
              value={
                canSubmit
                  ? 'Готов'
                  : `${invalidIssues} ${formatIssueCount(invalidIssues)}`
              }
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-black/10 px-4 py-3 lg:px-6">
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-black/65">
                  Нажмите на день, чтобы изменить смену. Итоги обновятся сразу.
                </p>
              </div>
              <SubmitButton canSubmit={canSubmit} onClick={handleSubmit} />
            </div>
      ...
      <div className="overflow-x-auto p-3 lg:p-4">
        ...
        <div className="grid min-w-[760px] grid-cols-7 gap-2 lg:min-w-0">
          {week.days.map((day) => {
            ...
            return (
              <ShiftCell key={toDateKey(day)} date={day} isMuted={isMuted} />
            )
          })}
        </div>
      </div>
