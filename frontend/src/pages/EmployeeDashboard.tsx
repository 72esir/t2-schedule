...
import type { StreakHistoryItem, User, UserStreak } from '../api/types';
...
export default function EmployeeDashboard({ user, onLogout }: EmployeeDashboardProps) {
  ...
  const summaryQuery = useMyScheduleSummaryQuery(user.is_verified && hasActivePeriod);
  const validationQuery = useMyScheduleValidationQuery(user.is_verified && hasActivePeriod);
  const changeRequestQuery = useMyChangeRequestQuery(user.is_verified && hasActivePeriod);
  const streakQuery = useMyStreakQuery(user.is_verified);
  const updateScheduleMutation = useUpdateMyScheduleMutation();
  const createRequestMutation = useCreateChangeRequestMutation();
  const redeemStreakMutation = useRedeemStreakMutation();
  ...
  const weekSummaries = useMemo(
    () => createWeekViewModels(periodStart, periodEnd, shiftsByDate, getRuleViolations),
    [getRuleViolations, periodEnd, periodStart, shiftsByDate],
  );
  const periodHours = weekSummaries.reduce((total, week) => total + week.totalHours, 0);
  ...
  const canEditSchedule =
    user.is_verified && hasActivePeriod && !currentRequest && !scheduleQuery.isPending;
  const isPendingSubmit = updateScheduleMutation.isPending || createRequestMutation.isPending;
  ...
  function handleRedeemStreak() {
    redeemStreakMutation.mutate(undefined, {
      onSuccess: (result) => {
        toast(
          `Вы получили ${result.awarded_bonus} бонусов. Текущий баланс: ${result.bonus_balance}.`,
        );
      },
      onError: (error) => toast(getApiErrorMessage(error), 'error'),
    });
        ...
        <header className="grid gap-4 rounded-md border border-black/10 bg-white px-4 py-4 lg:grid-cols-[1fr_auto] lg:items-center lg:px-6">
          ...
          <div className="grid gap-2 sm:min-w-[520px]">
            <div className="grid grid-cols-3 gap-2">
              <MetricTile Icon={Clock3} label="Часы" value={formatHours(displayPeriodHours)} />
              <MetricTile Icon={CalendarDays} label="Заполнено" value={`${plannedDays} дн.`} />
              <MetricTile
                Icon={ShieldCheck}
                label="Статус"
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 lg:px-5">
              ...
              {currentRequest && currentRequest.status === 'pending' && (
                <div className="mb-4 flex items-center gap-2 rounded-lg rounded-lg border border-black/10 bg-[#f7f7f8] px-4 py-3 text-sm font-bold text-black">
                  <FileQuestion size={16} aria-hidden="true" className="shrink-0 text-black/45" />
              ...
              {currentRequest && currentRequest.status === 'rejected' && (
                <div className="mb-4 flex items-center gap-2 rounded-lg border-2 border-[#ff3495] bg-white px-4 py-3 text-sm font-bold text-black">
                  <AlertCircle size={16} aria-hidden="true" className="shrink-0 text-[#ff3495]" />
                  Менеджер отклонил вашу заявку:{' '}
                  {currentRequest.manager_comment || 'Без комментария'}.
                </div>
              )}

              {periodRuleViolations.sixWorkingDaysInRow && (
                <div className="mb-4 flex items-center gap-2 rounded-lg border-2 border-[#ff3495] bg-white px-4 py-3 text-sm font-bold text-black">
                  <AlertCircle size={16} aria-hidden="true" className="shrink-0 text-[#ff3495]" />В
                  периоде есть 7 рабочих дней подряд. Добавьте выходной.
                </div>
              )}
                    ...
                    <div className="flex items-start gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs sm:text-sm font-bold text-black/60">
                          Система нашла {suggestedQuery.data.match_count} прошлых периода с
                          идентичным заполнением. Хотите применить этот шаблон?
                        </p>
                      </div>
                    </div>
              ...
              <StreakRewardsCard
                ...
                isLoading={streakQuery.isPending}
                isError={streakQuery.isError}
                errorText={streakQuery.isError ? getApiErrorMessage(streakQuery.error) : null}
                isRedeeming={redeemStreakMutation.isPending}
                onRedeem={handleRedeemStreak}
...
function StreakRewardsCard({
  ...
  onRedeem,
  onRetry,
}: StreakRewardsCardProps) {
  const hasRedeem = (streak?.redeemable_sets ?? 0) > 0;
      ...
      <div className="grid gap-5 p-4 lg:grid-cols-[1.1fr_0.9fr] lg:p-5">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex h-9 items-center gap-2 rounded-md bg-black px-3 text-xs font-black uppercase text-white">
              <Flame size={15} aria-hidden="true" />
              Ритм заполнения
            </span>
            <span className="rounded-md border border-black/10 bg-white px-3 py-2 text-xs font-black uppercase text-black/45">
              5 стриков = 10 бонусов
            </span>
          </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <RewardMetricTile
              Icon={Flame}
              label="Текущий стрик"
              value={isLoading ? '...' : String(streak?.current_streak ?? 0)}
              accent="pink"
            />
            <RewardMetricTile
              Icon={Trophy}
              label="Лучший стрик"
              value={isLoading ? '...' : String(streak?.longest_streak ?? 0)}
              accent="lime"
            />
            <RewardMetricTile
              Icon={Coins}
          ...
          {isError && errorText && (
            <div className="mt-4 flex flex-wrap items-center gap-2 rounded-lg border-2 border-[#ff3495] bg-white px-4 py-3 text-sm font-bold text-black">
              <AlertCircle size={16} aria-hidden="true" className="shrink-0 text-[#ff3495]" />
              <span className="min-w-0 flex-1">{errorText}</span>
              <button
                type="button"
                onClick={onRetry}
                className="inline-flex h-10 items-center justify-center rounded-md border border-black/10 px-3 text-xs font-black uppercase text-black transition hover:border-black">
                Повторить
              </button>
            </div>
          )}
          ...
          {!isLoading && !isError && streak && (
            <>
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                <InfoChip text={`${streak.completed_periods_count} успешных периодов`} />
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm font-bold text-black/70">
                      {hasRedeem
                        ? `Доступно обменов: ${streak.redeemable_sets}. Можно конвертировать стрик в бонусы прямо сейчас.`
                        : 'Для обмена нужно накопить минимум 5 стриков.'}
                    </p>
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={onRedeem}
                    disabled={!hasRedeem || isRedeeming}
                    className={`inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-md px-4 py-3 text-center text-sm font-black uppercase leading-tight transition focus:outline-none focus:ring-4 sm:w-auto sm:shrink-0 sm:px-5 ${
                      hasRedeem && !isRedeeming
                    ...
                    <span className="whitespace-normal break-words">
                      {isRedeeming ? 'Обмениваем стрик' : 'Обменять 5 стриков на 10 бонусов'}
                    </span>
                  </button>
                </div>
              </div>
        ...
        <div className="min-w-0 rounded-md border border-black/10 bg-white p-4">
          ...
          <p className="mt-1 text-sm font-bold text-black/65">
            Последние результаты по закрытию графиков.
          </p>
          ...
          {isLoading ? (
            <div className="mt-4 space-y-2">
              {Array.from({ length: 4 }, (_, index) => (
                <div
                  key={index}
                  className="h-14 animate-pulse rounded-lg border border-black/10 bg-[#f3f3f5]"
                />
              ))}
            </div>
            ...
            <div
              className={`mt-4 space-y-2 ${
                shouldScrollHistory ? 'max-h-[22rem] overflow-y-auto pr-1' : ''
              }`}>
              {streak.history.map((item) => (
                <StreakHistoryRow key={item.period_id} item={item} />
              ))}
            </div>
          ) : (
            <p className="mt-4 rounded-lg border border-dashed border-black/15 bg-[#f7f7f8] px-4 py-5 text-sm font-bold text-black/50">
              История стриков появится после первых оценённых периодов.
            </p>
          )}
        </div>
      </div>
...
function RewardMetricTile({
  ...
  return (
    <div className="rounded-md border border-black/10 bg-white p-4">
      ...
      <p className="mt-3 text-xs font-black uppercase tracking-[0.12em] text-black/45">{label}</p>
      <p className="mt-1 text-3xl font-black leading-none text-black">{value}</p>
    </div>
  );
}
...
function InfoChip({ text }: { text: string }) {
  return (
    <div className="rounded-md border border-black/10 bg-white px-3 py-2 text-xs font-black uppercase text-black/55">
      {text}
    </div>
  );
}

function StreakHistoryRow({ item }: { item: StreakHistoryItem }) {
  return (
    <div className="grid grid-cols-[auto_1fr] gap-3 rounded-lg border border-black/10 bg-[#f7f7f8] p-3">
      <div
        className={`mt-0.5 grid size-8 place-items-center rounded-full ${
          item.success ? 'bg-[#a7fc00] text-black' : 'bg-[#ff3495] text-white'
        }`}>
        {item.success ? (
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-sm font-black text-black">
            {format(parseISO(item.period_start), 'd MMM', { locale: ru })} -{' '}
            {format(parseISO(item.period_end), 'd MMM', { locale: ru })}
          </p>
          <span
            className={`rounded-md px-2 py-1 text-[10px] font-black uppercase ${
              item.success
                ? 'bg-[#a7fc00] text-black'
                : 'border border-[#ff3495] bg-white text-[#ff3495]'
            }`}>
            {item.success ? 'Зачтено' : 'Стрик прерван'}
          </span>
        </div>

        <p className="mt-1 text-xs font-bold text-black/55">
          Дедлайн: {format(parseISO(item.deadline), 'd MMM, HH:mm', { locale: ru })} ·{' '}
          {formatStreakReason(item.reason)}
        </p>
      </div>
    </div>
  );
...
function WeekSection({ week, weekIndex, periodStart, periodEnd, readOnly }: WeekSectionProps) {
  return (
    <article className="rounded-lg border border-black/10 bg-[#f7f7f8]">
      <div className="grid gap-3 border-b border-black/10 p-3 lg:grid-cols-[1fr_420px] lg:items-center lg:p-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-lg font-black uppercase leading-none">Неделя {weekIndex + 1}</h2>
    ...
    <div className="rounded-lg border border-black/10 bg-white p-3">
      <div className="mb-2 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-2xl font-black leading-none">{formatHours(week.totalHours)}</p>
        </div>
        <span
          className={`inline-flex h-9 items-center rounded-md px-3 text-[10px] sm:text-xs font-black uppercase ${
            week.isValid
              ? 'bg-[#a7fc00] text-black'
...
function MetricTile({ Icon, label, value, tone = 'default' }: MetricTileProps) {
  return (
    <div
      ...
      <div
        className={`mb-2 grid size-7 sm:size-8 shrink-0 place-items-center rounded-md bg-white text-black self-start`}>
        <Icon size={16} aria-hidden="true" />
      </div>
      <div className="min-w-0">
        <p
          className={`text-[9px] sm:text-xs font-black uppercase leading-tight truncate text-black/45`}>
          {label}
        </p>
        <p className={`mt-1 truncate text-base sm:text-xl font-black leading-none text-black`}>
          {value}
        </p>
      </div>
    </div>
...
function HintPill({ isValid, text }: { isValid: boolean; text: string }) {
  ...
  return (
    <span
      ...
      <Icon size={14} aria-hidden="true" className={isValid ? 'text-black/35' : 'text-[#ff3495]'} />
      {text}
    </span>
  );
}
...
function hasWorkingDaysInRowViolation(days: Date[], schedule: ScheduleByDate): boolean {
...
function countPlannedDays(schedule: ScheduleByDate, periodStart: Date, periodEnd: Date): number {
...
function formatStreakReason(reason: string): string {
  switch (reason) {
    case 'on_time':
      return 'Период закрыт вовремя';
    case 'late':
      return 'Период закрыт с опозданием';
    case 'missed':
      return 'Период не был заполнен вовремя';
    case 'no_schedule':
      return 'График не был заполнен';
    case 'saved_after_deadline':
      return 'График был сохранён после дедлайна';
    case 'late_change_request':
    case 'change_request_after_deadline':
      return 'После дедлайна была подана заявка на изменение';
    default:
